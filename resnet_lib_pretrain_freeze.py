...
MODEL_DIR = "checkpoints_pretrain_freeze"
BEST_MODEL_PATH = os.path.join(MODEL_DIR, "best_resnet18.pth")

os.makedirs(MODEL_DIR, exist_ok=True)
os.makedirs("output_pretrain_freeze", exist_ok=True)
...
# ================= 日志系统（不影响 tqdm） =================
LOG_PATH = os.path.join("output_pretrain_freeze", "train.log")
...
class ResNetClassifier(nn.Module):
    def __init__(self, num_classes):
        ...
        for name, param in self.backbone.named_parameters():
            # 只让 fc 层的参数保持可训练，其他参数全部冻结
            if not name.startswith('fc.'):  # 匹配 fc.weight 和 fc.bias
...
def draw_train_plot(list_train_acc, list_val_acc, list_train_loss):
    ...
    plt.savefig('output_pretrain_freeze/training_curves.png')
    plt.show()
...
def plot_confusion_matrix(model, test_loader, class_names):
    ...
    plt.savefig("output_pretrain_freeze/confusion_matrix.png")
    plt.show()
