...
#日志
LOG_PATH = os.path.join("output_cnn_init", "train.log")
...
def draw_train_plot(list_train_acc, list_val_acc, list_train_loss):
    ...
    plt.savefig('output_cnn_init/training_curves.png')
    plt.show()
...
def train_net(model, lr, num_epochs, train_loader, val_loader,class_weights):
    ...
    for epoch in range(num_epochs):
        ...
        if verify_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'best_model_cnn_init.pth')
        #更新学习率
        scheduler.step()
...
def plot_confusion_matrix(model, test_loader, class_names):
    ...
    plt.savefig("output_cnn_init/confusion_matrix.png")
    plt.show()
...
def main():
    ...
    best_model_path = "best_model_cnn_init.pth"
