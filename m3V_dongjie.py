...
BEST_MODEL_PATH = os.path.join(MODEL_DIR, "best_mV3Large_dongjie.pth")
...
class MobileNetV3LargeClassifier(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        ...
        # 第一步：冻结主干网络所有参数（将requires_grad设为False）
        for param in self.base_model.parameters():
            param.requires_grad = False
        # 第二步：解冻分类器参数（保留分类器可训练，仅训练这部分）
...
def main():
    ...
    # 模型
    num_classes = 10
    model = MobileNetV3LargeClassifier(num_classes).to(DEVICE)
